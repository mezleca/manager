import { create_tag } from "../extra/tag.js";
import { create_dropdown } from "../extra/dropdown.js";
import { create_range } from "../extra/range.js";
import { create_element, safe_id, safe_text } from "./utils.js";
import { ipc } from "../../ipc/message.js";
import { create_collection_card } from "../cards/collection.js";
    ...
    if (options.html) {
        content.querySelectorAll('a[href^="http"]').forEach((a) => {     
            a.addEventListener("click", (element) => {
                ...
                ipc.send("open", { url: element.target.href });
            });
        });
    } else {
    ...
    for (let i = 0; i < elements.length; i++) {
        ...
        switch (type) {
            ...
            case 'cards': {
                ...
                for (let j = 0; j < props.length; j++) {
                    const card = props[j];
                    const card_data = create_collection_card(crypto.randomUUID(), card.name);
                    
                    if (card.selectable) {
                        card_data.container.addEventListener("click", () => {
                            card_data.container.classList.toggle("selected");
                        });
                    } else {
                        card_data.name.textContent += " (already imported)";
                    }
                    
                    card_data.count.textContent = ` ${card.count || 0} maps`;
                    cards.add(card_data);
                    container_cards.appendChild(card_data.container);
                }
